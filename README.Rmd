...
## Example run
Let's write the toy example presented in the paper. First the incidence matrix:

```{r 'toy_matrix', eval = TRUE, message=FALSE, warning=FALSE, out.width="40%",  fig.align = 'center'}
...
Now let's invent a correlation matrix that reflects Figure 1 of the paper.

```{r 'cormat_matrix', eval = TRUE, message=FALSE, warning=FALSE, out.width="40%",  fig.align = 'center'}
library(ggplot2)
...
The first thing we can do is check whether the probability values generated using the PGLMM (provided by the function `probability_estimation`) follow a similar profile to that of the example in Figure 1.

```{r 'probability_profile', eval = TRUE, message=FALSE, warning=FALSE, out.width="30%",  fig.align = 'center'}
library(RESOLDRE)
...
Then, we can use the function `resoldre()` to randomize the matrix either using the new `pmat` or direclty imputing the correlation matrix `cormed`:

```{r 'randomization', eval = F, message=FALSE, warning=FALSE}
library(RESOLDRE)
rmat <- resoldre(mat = mat, cormed = cormed, perspective = "columns", bipartite = T)
```

Notice that we use the option `bipartite = T` to indicate that this is not a square matrix. 

For the sake of showcasing what the algorithm does, we can calculate the ratio between the number of times that we find the link (c-E) relative to the link (c-A) when using the informed randomized strategy and the uninformed strategy.

```{r 'comparison', eval = TRUE, message=FALSE, warning=FALSE, out.width="30%",  fig.align = 'center'}
library(dplyr)
library(knitr)
